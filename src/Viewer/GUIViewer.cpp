...
void OTEViewer::InitMenu()
{
	...
	menu.callback_draw_viewer_menu = [&]() {
		if (ImGui::CollapsingHeader("MeshIO", ImGuiTreeNodeFlags_DefaultOpen))
		{
		...
		if (ImGui::CollapsingHeader("Cutting System", ImGuiTreeNodeFlags_DefaultOpen))
		{
			if (ImGui::Button("Load Marker", ImVec2(-1, 0)))
			{
				LoadMarker();
			}
			if (ImGui::Button("Save Marker", ImVec2(-1, 0)))
			{
				SaveMarker();
			}
			ImGui::InputDouble("double", &cone_angle_, 0, 0, "%.4f");
			if (ImGui::Button("Add Cone", ImVec2(-1, 0)))
			{
				SetSingularity(cone_angle_); 
				UpdateMeshViewer();
			}
			if (ImGui::Button("Add Slice", ImVec2(-1, 0)))
			{
				SetSlice(); 
				UpdateMeshViewer();
			}
			if (ImGui::Button("Reset Marker", ImVec2(-1, 0)))
			{
				marker_.ResetMarker(); 
		...
		{
			if (ImGui::Button("Euclidean Orbifold", ImVec2(-1, 0)))
			{
			...
			if (ImGui::Button("Hyperbolic Orbifold", ImVec2(-1, 0)))
			{
			...
			if (ImGui::Button("Hilbert BFF With K", ImVec2(-1, 0)))
			{
			...
			if (ImGui::Button("Harmonic BFF With K", ImVec2(-1, 0)))
			{
				BFFSolver solver(this->mesh_, marker_.GetSingularityFlag(), marker_.GetConeAngleFlag(), marker_.GetSliceFlag());
			...
			if (ImGui::Button("Hilbert BFF With Free B", ImVec2(-1, 0)))
			{
				BFFSolver solver(this->mesh_, marker_.GetSingularityFlag(), marker_.GetConeAngleFlag(), marker_.GetSliceFlag());
			...
			if (ImGui::Button("Harmonic BFF With Free B", ImVec2(-1, 0))){
				BFFSolver solver(this->mesh_, marker_.GetSingularityFlag(), marker_.GetConeAngleFlag(), marker_.GetSliceFlag());
			...
			if (ImGui::Button("Harmonic BFF With Cones", ImVec2(-1, 0))) {
				BFFSolver solver(this->mesh_, marker_.GetSingularityFlag(), marker_.GetConeAngleFlag(), marker_.GetSliceFlag());
			...
			if (ImGui::Button("Hilbert BFF With Cones", ImVec2(-1, 0))) {
				BFFSolver solver(this->mesh_, marker_.GetSingularityFlag(), marker_.GetConeAngleFlag(), marker_.GetSliceFlag());
		...
		{
			if (ImGui::Combo("Show Option", (int *)(&show_option_), "Original\0Sliced\0Embedding\0Embedding\0Covering Space\0\0"))
			{
				UpdateMeshViewer();
			}
			if (ImGui::Checkbox("Show Boundaries", &show_boundaries_))
			{
				UpdateMeshViewer();
			}

			if (ImGui::Checkbox("Show Slices and Cones", &show_slice_))
			{
				UpdateMeshViewer();
			}

			if (ImGui::Checkbox("Show Vertex Labels", &show_vertex_labels_))
			{
				UpdateMeshViewer();
			}
		}
...
{
	callback_key_down = [this](igl::opengl::glfw::Viewer& viewer, unsigned char key, int modifier)
	{
		if (key == 'S') {
	...
	callback_key_up = [this](igl::opengl::glfw::Viewer& viewer, unsigned char key, int modifier)
	{
		if (key == 'S') {
